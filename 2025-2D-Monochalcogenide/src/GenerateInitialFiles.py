...
def log_register(string):
    ...
    path_log_file = os.path.join(C.PATH_FOLDER_LOG, 'simulation_dos_soc_generation.log')
    with open(path_log_file, 'a') as log:
        log.write(date_time_formatted + ' ' + string + '\n')


def create_structure_folders():
    path_output = os.path.join(C.PATH_FOLDER_SIMULATION_FILES, 'simulation_dos_soc')
    ...
    for dos_sim in chemical_compounds_structures:
        log_register(f'DOS-SOC Simulations: {dos_sim[0]} - {dos_sim[1]}')
    ...
    _path_json = os.path.join(C.PATH_FOLDER_JSON_FILES, 'json_dos_soc_simulation.json')
...
def incar_read(path_files):
    ...
    nelect_soc = 2*nelect
    _line_pos_nedos = f'NBANDS = {nelect_soc}\n'  # NELECT - OUTCAR
    _line_pos_nbands = 'LSORBIT = .TRUE.\n'
    ...
    for line_number, content in enumerate(lines, start=1):
        if re.search(_match_ispin, content):
            ...
            incar_content += _line_pos_nbands
